...
def normalize_refs(refs: Iterable[Any]) -> List[ImageRef]:
    ...
    兼容两种输入形态：``{"pdf_id": ..., "page": ...}`` 字典，以及
    ``(pdf_id, page)`` 二元组/列表。之所以要兼容，是因为 state 经 checkpointer
    的 JsonPlusSerializer 落盘后，tuple 会被序列化成 list 取回。
    """
    out: List[ImageRef] = []
    for ref in refs or []:
        pdf_id: Any = ""
...
def render_image_section(refs: Iterable[Any], *, heading: str = "【教材原图】") -> str:
